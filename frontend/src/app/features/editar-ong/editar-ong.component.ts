...
export class EditarOngComponent implements OnInit, OnDestroy, AfterViewInit {
  @ViewChild(ModalMensagemComponent) modalMensagem!: ModalMensagemComponent;

  perfil?: CarregarDadosOng;
  ...
  ngOnInit(): void {
    this.perfilForm = this.fb.group({
      nomeEntidade: [{ value: '', disabled: true }],
  ...
  private carregarDados(): void {
    this.ongService.DadosOng()
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: dto => {
          this.perfil = dto;
          this.perfilForm.patchValue({
            nomeEntidade: dto.nomeEntidade,
            email: dto.email,
            cnpj: dto.cnpj,
        ...
        error: err => console.error('Erro ao carregar dados da ONG:', err)
      });
  }
  ...
  private verificarAlteracoes() {
    ...
    const endAlt = ['cep', 'estado', 'cidade', 'bairro', 'rua', 'numero']
      .some(f => (this.perfilForm.get(f)!.value || '').trim()
        !== (this.perfil![f as keyof CarregarDadosOng] || '').trim());
  ...
  salvar(): void {
    ...
    const endAlt = camposEnd.some(c =>
      ...
      !== (this.perfil![c as keyof CarregarDadosOng] || '').trim()
    );
    if (endAlt) {
      payload.endereco = {};
      camposEnd.forEach(c => {
    ...
    this.ongService.atualizarOng(payload)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: () => {
